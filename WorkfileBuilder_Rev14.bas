...
Private Sub CreateWorkingSheet(hasCEIDsSheet As Boolean, hasMilestonesSheet As Boolean, _
    ...
    ' Rev14: Sort by Status + Definitions sort priorities (same sort as TIS Update)
    stepDesc = "StatusSort"
    Application.StatusBar = "Building Working Sheet... 88% - Sorting by status and start date"
    TISLoader.SortWorkingSheet newSheet
...
Public Sub PopulateHealthColumn(ws As Worksheet, outputColMap As Object, _
    ...
    ' dataStartRow = header row (e.g. 14 or 15 depending on caller).
    ' lastDataRow = last data row in sheet.
    Dim lastDataRow As Long
    lastDataRow = dataStartRow + dataRowCount
    If lastDataRow <= dataStartRow Then Exit Sub
    ...
    ' --- Status row CF: Cancelled = red text, other non-active = orange text ---
    ' Applied to the entire data row range so the visual signal spans all columns.
    If outputColMap.exists(LCase(TIS_COL_STATUS)) Then
        Dim stCol As Long
        stCol = outputColMap(LCase(TIS_COL_STATUS))
        Dim stAddr As String
        stAddr = "$" & TISCommon.ColLetter(stCol) & (dataStartRow + 1)

        Dim dataEndRow As Long
        dataEndRow = dataStartRow + dataRowCount
        Dim lastC As Long
        lastC = ws.Cells(dataStartRow, ws.Columns.Count).End(xlToLeft).Column
        If lastC < stCol Then lastC = stCol

        Dim rowRng As Range
        Set rowRng = ws.Range(ws.Cells(dataStartRow + 1, 1), ws.Cells(dataEndRow, lastC))

        On Error Resume Next
        ' Cancelled: red font on entire row
        Dim sfc As FormatCondition
        Set sfc = rowRng.FormatConditions.Add(Type:=xlExpression, _
            Formula1:="=" & stAddr & "=""Cancelled""")
        If Err.Number = 0 And Not sfc Is Nothing Then
            sfc.Font.Color = RGB(185, 28, 28)    ' dark red
            sfc.Font.Strikethrough = True
            sfc.StopIfTrue = False
        Else: Err.Clear
        End If

        ' Completed: muted gray text
        Set sfc = rowRng.FormatConditions.Add(Type:=xlExpression, _
            Formula1:="=" & stAddr & "=""Completed""")
        If Err.Number = 0 And Not sfc Is Nothing Then
            sfc.Font.Color = RGB(100, 116, 139)  ' slate gray
            sfc.StopIfTrue = False
        Else: Err.Clear
        End If
        ...
        ' On Hold: orange text
        Set sfc = rowRng.FormatConditions.Add(Type:=xlExpression, _
            Formula1:="=" & stAddr & "=""On Hold""")
        If Err.Number = 0 And Not sfc Is Nothing Then
            sfc.Font.Color = RGB(217, 119, 6)    ' amber/orange
            sfc.Font.Bold = True
            sfc.StopIfTrue = False
        Else: Err.Clear
        End If
    ...
    For idx = LBound(milestoneKeys) To UBound(milestoneKeys)
        ...
        If group.exists(1) And group.exists(2) Then
            ...
            If startColIdx > 0 And endColIdx > 0 Then
                ...
                ' Returns 0 instead of "" to avoid green triangle "inconsistent formula"
                ' warnings. Number format "0;-0;;@" hides zero values visually.
                r = dataStartRow + 1
        ...
        ' Format: show positive/negative numbers, hide zero (blank appearance)
...
Private Sub InstallSheetEvents(ws As Worksheet)
    ...
    ' Lock? Worksheet_Change handler removed -- sheet protection not used.
    ' Lock? enforcement relies on Data Validation only.
...
Public Sub ActivateWhatIfMode()
    ...
    ' Bulk-read Our Dates, Status, and WhatIf columns (3 COM calls total,
    ' replaces 2N cell-by-cell reads in the old shift + write loops)
    Dim ourData As Variant
    ourData = ws.Range(ws.Cells(headerRow + 1, ourStartCol), _
                        ws.Cells(lastRow, ourEndCol)).Value

    Dim statusArr As Variant, wifArr As Variant
    If statusCol > 0 Then
        statusArr = ws.Range(ws.Cells(headerRow + 1, statusCol), _
                              ws.Cells(lastRow, statusCol)).Value
    End If
    wifArr = ws.Range(ws.Cells(headerRow + 1, whatIfCol), _
                       ws.Cells(lastRow, whatIfCol)).Value
    ...
    For r = 1 To rowCount
        ' Skip non-Active rows (array read -- zero COM calls)
        If statusCol > 0 Then
            If LCase(Trim(CStr(statusArr(r, 1) & ""))) <> "active" Then GoTo NextWIFRow
        End If
...
NextWIFRow:
    ...
    ' Write shifted Our Dates back -- single bulk write of entire array
    ws.Range(ws.Cells(headerRow + 1, ourStartCol), _
             ws.Cells(lastRow, ourEndCol)).Value = ourData
    ws.Range(ws.Cells(headerRow + 1, ourStartCol), _
             ws.Cells(lastRow, ourEndCol)).NumberFormat = "mm/dd/yyyy"

    ' --- Step 3: Recalculate (no Gantt rebuild needed) ---
    ' Gantt formulas reference Our Date cells directly. Changing the values
    ' triggers automatic recalculation -- no need to clear and rebuild the
    ' entire Gantt structure, headers, and CF rules. Just recalc the sheet.
    Application.Calculation = xlCalculationAutomatic
    ws.Calculate
...
Public Sub DeactivateWhatIfMode()
    ...
    ' Delete backup sheet + purge any accumulated WhatIf_Consumed_* sheets
    On Error Resume Next
    ws.Activate
    Application.DisplayAlerts = False
    ...
    ' Purge accumulated WhatIf_Consumed_* sheets from previous failed deletes
    Dim purgeIdx As Long
    For purgeIdx = ThisWorkbook.Sheets.Count To 1 Step -1
        If Left(ThisWorkbook.Sheets(purgeIdx).Name, 16) = "WhatIf_Consumed_" Or _
           Left(ThisWorkbook.Sheets(purgeIdx).Name, 14) = "WhatIf_Backup" Then
            ThisWorkbook.Sheets(purgeIdx).Visible = xlSheetVisible
            ThisWorkbook.Sheets(purgeIdx).Delete
        End If
    Next purgeIdx
    If Err.Number <> 0 Then Err.Clear
    ...
    ' Recalculate (no Gantt rebuild needed -- formulas reference Our Date cells
    ' directly, so restoring the values triggers automatic recalculation)
    Application.Calculation = xlCalculationAutomatic
    ws.Calculate
