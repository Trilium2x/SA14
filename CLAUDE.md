...
| H23 | `TISLoader` | Sort bug: `SortWorkingSheet` used `End(xlUp)` on column 1 for `wsLastRow`. If `ListObject.Resize` failed silently during append (masked by `On Error Resume Next`), new rows fell outside the sort range. Fixed: uses ListObject boundary (primary) with `End(xlUp)` fallback. Added `DebugLog` after masked `Resize` failures. |
| H24 | `WorkfileBuilder` | WhatIf performance: eliminated ~2300 COM calls. Bulk-read Status + WhatIf columns alongside Our Dates (3 array reads instead of 2N cell reads). Merged shift + write into single pass. Single bulk write of entire `ourData` array replaces per-row write loop. Saves ~1.4s at 500 rows. |
| H25 | `GanttBuilder` | Gantt border performance: replaced per-row `Borders(xlEdgeBottom)` loop (N COM calls) with single `Borders(xlInsideHorizontal)` call on the full range. Saves ~2500 COM calls at 500 rows. Header map now built via bulk array read (single COM call). |
| H26 | Multiple | Encoding fix: ALL 9 `.bas` files cleaned to pure ASCII. DashboardBuilder had **21 code lines** with UTF-8 smart quotes (compile-breaking when imported via VBA editor). RampAlignment had em-dashes in string literals. All other files had em-dashes/arrows in comments. Replaced: smart quotes -> straight quotes, em-dashes -> `--`, arrows -> `->`. |
| H27 | `TISLoader` | `SortWorkingSheet` now uses Definitions sort priorities (Column C) as secondary sort within each Status group. Previously hardcoded MIN(Our Dates) only. Definitions priorities are read via `GetSortPriorityColumns`, with Our Date redirect (TIS headers map to Our Date columns). Fallback to MIN(Our Dates) if no Definitions priorities exist. Helper column cleanup updated to delete all helpers. |
| H28 | `WorkfileBuilder` | Status row CF added in `PopulateHealthColumn`: Cancelled=red+strikethrough, Completed=gray, On Hold=orange bold, Non IQ=orange. Applied to full data row range. `StopIfTrue=False` so Health CF still shows. Runs on every build, TIS load, and WhatIf toggle. |
| H29 | `HCHeatmap` | "Toggle already exists" MsgBox suppressed when called programmatically (`targetSheet` provided). Previously blocked TIS load flow with a modal dialog requiring user click. |
| H30 | `TISLoader` | New systems appended during TIS load now get ALL formula columns filled. Previously only Health formula was copied to new rows. Group VLOOKUP, Actual Duration, STD Duration, Gap, New/Reused, and any other formula columns were blank. Fixed: generic scan of first data row for `.HasFormula` cells, copies formulas down to appended rows via `PasteSpecial xlPasteFormulas`. |
