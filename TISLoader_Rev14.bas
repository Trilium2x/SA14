...
'====================================================================
' LOAD AND COMPARE TIS (Steps 1-3 only)
' Loads new TIS file, backs up old TIS, generates TIScompare.
' Does NOT touch the Working Sheet. Call ApplyTISToWorkingSheet separately.
'====================================================================

Public Sub LoadAndCompareTIS()
    On Error GoTo ErrorHandler
    ...
    If CheckTISExists() Then
        ' Step 1: Backup current TIS to TISold
        BackupTISToTISold

        ' Step 2: Load new TIS file
        Dim loaded As Boolean
        loaded = LoadNewTISIntoSheet()
        If Not loaded Then GoTo Cleanup

        ' Step 3: Generate TIScompare
        Dim filters As Object
        Set filters = GetDefinitionsFilters()
        If SheetExists(ThisWorkbook, SHEET_TISOLD) Then
            CompareTISWorkflow filters
        ...
        ' Update TIS status on Controls sheet (write to cells via named ranges)
        On Error Resume Next
        Dim tisRowCount As Long
        tisRowCount = ThisWorkbook.Sheets(SHEET_TIS).Cells(ThisWorkbook.Sheets(SHEET_TIS).Rows.Count, 1).End(xlUp).Row - 1
        Dim statusDateCell As Range, statusSyncCell As Range
        Set statusDateCell = Nothing: Set statusSyncCell = Nothing
        Set statusDateCell = ThisWorkbook.Names("TIS_STATUS_DATE").RefersToRange
        Set statusSyncCell = ThisWorkbook.Names("TIS_STATUS_SYNC").RefersToRange
        If Not statusDateCell Is Nothing Then statusDateCell.Value = Format(Now, "yyyy-mm-dd") & " | " & tisRowCount & " rows"
        If Not statusSyncCell Is Nothing Then statusSyncCell.Value = "No"
        If Err.Number <> 0 Then Err.Clear
        On Error GoTo ErrorHandler

        Application.ScreenUpdating = True
        MsgBox "TIS loaded and compared." & vbCrLf & vbCrLf & _
               "Review the TIScompare sheet, then click" & vbCrLf & _
               "'Update TIS to WS' to apply changes.", vbInformation, "TIS Load Complete"
        Application.ScreenUpdating = False
    Else
        Application.ScreenUpdating = True
        MsgBox "No TIS currently loaded. Loading first TIS...", vbInformation
        Application.ScreenUpdating = False
        LoadFirstTIS
    End If
...
ErrorHandler:
    MsgBox "Error in LoadAndCompareTIS: " & Err.Description, vbCritical
    DebugLog "LoadAndCompareTIS ERROR: " & Err.Description
...
'====================================================================
' APPLY TIS TO WORKING SHEET (Step 4 with preview)
' Pre-checks: Working Sheet and TIS must exist.
' Shows preview of changes before applying.
'====================================================================

Public Sub ApplyTISToWorkingSheet()
    On Error GoTo ErrorHandler

    ' Pre-check: Working Sheet must exist
    Dim ws As Worksheet
    Set ws = TISCommon.FindWorkingSheet()
    If ws Is Nothing Then
        MsgBox "No Working Sheet found." & vbCrLf & vbCrLf & _
               "Run 'Build Working Sheet' first, or load a TIS and use the full workflow.", _
               vbExclamation, "Update TIS to WS"
        Exit Sub
    End If

    ' Pre-check: TIS sheet must be loaded
    If Not SheetExists(ThisWorkbook, SHEET_TIS) Then
        MsgBox "No TIS data loaded." & vbCrLf & vbCrLf & _
               "Click 'Load/Compare TIS' first.", vbExclamation, "Update TIS to WS"
        Exit Sub
    End If
    ...
    ' Call the existing update logic (it has its own AppState management)
    Application.StatusBar = "Updating Working Sheet from TIS..."
    UpdateWorkingSheetFromTIS filters

    ' Mark as synced on Controls sheet
    On Error Resume Next
    Dim syncCell As Range
    Set syncCell = ThisWorkbook.Names("TIS_STATUS_SYNC").RefersToRange
    If Not syncCell Is Nothing Then syncCell.Value = "Yes"
    If Err.Number <> 0 Then Err.Clear
    On Error GoTo 0
...
ErrorHandler:
    MsgBox "Error in ApplyTISToWorkingSheet: " & Err.Description, vbCritical
    DebugLog "ApplyTISToWorkingSheet ERROR: " & Err.Description
End Sub
...
NextTisMap:
    ...
    If newSystemRows.Count > 0 Then
        ...
        If ws.ListObjects.Count > 0 Then
            ...
            If appendRow + newRowCount - 1 > loEndRow Then
                ...
                If Err.Number <> 0 Then
                    DebugLog "WARNING: ListObject.Resize failed: " & Err.Description & _
                             " (appendRow=" & appendRow & ", newRowCount=" & newRowCount & ")"
                    Err.Clear
                End If
                On Error GoTo 0
            End If
        ...
        ' Row 15 may have a text sub-header, so scan rows 15-16 for the first formula.
        Dim healthCol As Long
        healthCol = 0
        If wsHeaderMap.exists(LCase(TIS_COL_HEALTH)) Then healthCol = wsHeaderMap(LCase(TIS_COL_HEALTH))
        If healthCol > 0 Then
            Dim firstDataRow As Long
            firstDataRow = wsHdr + 1  ' row 15
            ' Find the first row with a Health formula (may be row 15 or 16)
            If Not ws.Cells(firstDataRow, healthCol).HasFormula Then
                If ws.Cells(firstDataRow + 1, healthCol).HasFormula Then firstDataRow = firstDataRow + 1
            End If
        ...
        ' Extend ALL formula columns to new rows.
        ' Scans rows 15-16 for cells with formulas (Group VLOOKUP, Actual Duration,
        ' STD Duration, Gap, New/Reused, Health, etc.) and copies them down to new rows.
        ' Row 15 may have text sub-headers for duration columns, so fallback to row 16.
        Dim fmtFirstRow As Long
        fmtFirstRow = wsHdr + 1
        Dim fcScan As Long
        On Error Resume Next
        For fcScan = 1 To wsMaxCol
            Dim fmtSrcRow As Long
            fmtSrcRow = fmtFirstRow
            ' If row 15 has no formula, check row 16 (sub-header row pattern)
            If Not ws.Cells(fmtSrcRow, fcScan).HasFormula Then
                If ws.Cells(fmtSrcRow + 1, fcScan).HasFormula Then fmtSrcRow = fmtSrcRow + 1
            End If
            If ws.Cells(fmtSrcRow, fcScan).HasFormula Then
                ws.Cells(fmtSrcRow, fcScan).Copy
                ws.Range(ws.Cells(appendRow, fcScan), _
                         ws.Cells(appendRow + newRowCount - 1, fcScan)).PasteSpecial xlPasteFormulas
            End If
        Next fcScan
        Application.CutCopyMode = False
        If Err.Number <> 0 Then
            DebugLog "WARNING: Formula fill-down for new rows: " & Err.Description
            Err.Clear
        End If
        On Error GoTo UpdateErrorHandler
    ...
    ' 8. Reapply Health formula + CF to full data range (including new rows).
    '    Health CF and Status row CF are applied to a fixed range during Build WS.
    '    New rows appended here fall outside that range. PopulateHealthColumn
    '    rewrites the formula and CF to cover the entire current data extent.
    On Error Resume Next
    Dim healthDataCount As Long
    If ws.ListObjects.Count > 0 Then
        healthDataCount = ws.ListObjects(1).Range.Rows.Count  ' includes header
    Else
        healthDataCount = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row - wsHdr
    End If
    WorkfileBuilder.PopulateHealthColumn ws, wsHeaderMap, wsHdr, healthDataCount
    If Err.Number <> 0 Then
        DebugLog "WARNING: PopulateHealthColumn failed after TIS update: " & Err.Description
        Err.Clear
    End If
    On Error GoTo 0
...
' SORT WORKING SHEET
' Sort order:
'   1. Status (custom: Active=1, Completed=2, On Hold=3, Non IQ=4, Cancelled=5)
'   2. Definitions sort priorities (Column C) -- configurable multi-column date sort
'   3. Fallback: MIN(Our Date columns) if no Definitions priorities exist
'====================================================================

Public Sub SortWorkingSheet(ws As Worksheet)
    Dim wsHdr As Long
    wsHdr = TISCommon.FindHeaderRow(ws)
    If wsHdr = 0 Then Exit Sub
    ' Use ListObject boundary (primary) -- End(xlUp) on column 1 is fragile
    Dim wsLastRow As Long
    If ws.ListObjects.Count > 0 Then
        Dim loSort As ListObject
        Set loSort = ws.ListObjects(1)
        wsLastRow = loSort.Range.Row + loSort.Range.Rows.Count - 1
    Else
        wsLastRow = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    End If
    ...
    ' --- Build WS header map for Definitions sort lookup ---
    Dim wsHeaderMap As Object
    Set wsHeaderMap = CreateObject("Scripting.Dictionary")
    Dim hdrArr As Variant
    If wsMaxCol >= 1 Then
        hdrArr = ws.Range(ws.Cells(wsHdr, 1), ws.Cells(wsHdr, wsMaxCol)).Value
        Dim hc As Long
        For hc = 1 To wsMaxCol
            Dim hvs As String
            hvs = LCase(Trim(CStr(hdrArr(1, hc) & "")))
            If hvs <> "" And Not wsHeaderMap.exists(hvs) Then wsHeaderMap(hvs) = hc
        Next hc
    End If

    ' --- Helper columns start after all existing columns ---
    Dim nextHelper As Long
    nextHelper = wsMaxCol + 1

    ' Helper 1: Status sort (numeric: Active=1 ... Cancelled=5)
    Dim statusHelperCol As Long
    statusHelperCol = nextHelper
    nextHelper = nextHelper + 1
    ...
    ReDim statusSortArr(1 To dataRowCount, 1 To 1)
    For sr = 1 To dataRowCount
    ...
    ' --- Definitions sort priorities (secondary sort within each Status group) ---
    Dim sortDict As Object
    Set sortDict = TISCommon.GetSortPriorityColumns()
    Dim defHelperStart As Long
    defHelperStart = nextHelper
    Dim defHelperCount As Long
    defHelperCount = 0

    If sortDict.Count > 0 Then
        ' Build helper columns from Definitions sort priorities
        ' (same logic as TISCommon.SortWithHelperColumns but inline for the sort)
        Dim sortKeys As Variant
        sortKeys = sortDict.keys
        TISCommon.ShellSortVariantArray sortKeys

        Dim ski As Long
        For ski = LBound(sortKeys) To UBound(sortKeys)
            If Not sortDict.exists(sortKeys(ski)) Then GoTo NextDefKey

            Dim sortHeaderNames() As String
            sortHeaderNames = Split(sortDict(sortKeys(ski)), "|")
            If UBound(sortHeaderNames) < 0 Then GoTo NextDefKey

            Dim curHelper As Long
            curHelper = defHelperStart + defHelperCount
            defHelperCount = defHelperCount + 1
            ws.Cells(wsHdr, curHelper).Value = "_DefSort_" & CStr(sortKeys(ski))

            Dim defArr() As Variant
            ReDim defArr(1 To dataRowCount, 1 To 1)

            If UBound(sortHeaderNames) = 0 Then
                ' Single column priority
                Dim singleCol As Long
                singleCol = 0
                If wsHeaderMap.exists(LCase(sortHeaderNames(0))) Then singleCol = wsHeaderMap(LCase(sortHeaderNames(0)))
                ' Also check Our Date redirect: if TIS header maps to Our Date
                If singleCol = 0 Then
                    Dim ourRedirect As String
                    ourRedirect = GetOurDateForTISHeader(LCase(sortHeaderNames(0)))
                    If ourRedirect <> "" And wsHeaderMap.exists(ourRedirect) Then singleCol = wsHeaderMap(ourRedirect)
                End If
                If singleCol > 0 Then
                    Dim srcCol As Variant
                    srcCol = ws.Range(ws.Cells(firstDataRow, singleCol), ws.Cells(wsLastRow, singleCol)).Value
                    Dim scr As Long
                    For scr = 1 To dataRowCount
                        defArr(scr, 1) = srcCol(scr, 1)
                    Next scr
                End If
            Else
                ' Multi-column priority: take MIN date across columns
                Dim colArrays() As Variant
                ReDim colArrays(0 To UBound(sortHeaderNames))
                Dim validCols As Long
                validCols = 0
                Dim cj As Long
                For cj = 0 To UBound(sortHeaderNames)
                    Dim colIdx As Long
                    colIdx = 0
                    If wsHeaderMap.exists(LCase(sortHeaderNames(cj))) Then colIdx = wsHeaderMap(LCase(sortHeaderNames(cj)))
                    If colIdx = 0 Then
                        Dim ourRd2 As String
                        ourRd2 = GetOurDateForTISHeader(LCase(sortHeaderNames(cj)))
                        If ourRd2 <> "" And wsHeaderMap.exists(ourRd2) Then colIdx = wsHeaderMap(ourRd2)
                    End If
                    If colIdx > 0 Then
                        colArrays(cj) = ws.Range(ws.Cells(firstDataRow, colIdx), ws.Cells(wsLastRow, colIdx)).Value
                ...
                If validCols > 0 Then
                    Dim mr As Long
                    For mr = 1 To dataRowCount
                        Dim minVal As Date
                        minVal = DateSerial(2099, 12, 31)
                        Dim mj As Long
                        For mj = 0 To UBound(sortHeaderNames)
                            If Not IsEmpty(colArrays(mj)) Then
                                Dim dv As Variant
                                dv = colArrays(mj)(mr, 1)
                                If IsDate(dv) Then
                                    If CDate(dv) < minVal Then minVal = CDate(dv)
                                End If
                            End If
                        Next mj
                        If minVal < DateSerial(2099, 12, 31) Then defArr(mr, 1) = minVal
                    Next mr
                End If
            End If

            ws.Range(ws.Cells(firstDataRow, curHelper), ws.Cells(wsLastRow, curHelper)).Value = defArr
NextDefKey:
        Next ski
    End If

    ' Fallback: if no Definitions priorities, add MIN(Our Dates) as secondary sort
    Dim fallbackCol As Long
    fallbackCol = 0
    If defHelperCount = 0 Then
        fallbackCol = defHelperStart
        nextHelper = defHelperStart + 1
        ws.Cells(wsHdr, fallbackCol).Value = "_DateFallback"
        ...
        For oi = LBound(ourDateHeaders) To UBound(ourDateHeaders)
            ...
            End If
        Next oi
        If minFirstCol > 0 Then
            ws.Cells(firstDataRow, fallbackCol).Formula = "=MIN(" & _
                TISCommon.ColLetter(minFirstCol) & firstDataRow & ":" & _
                TISCommon.ColLetter(minLastCol) & firstDataRow & ")"
            If wsLastRow > firstDataRow Then
                ws.Cells(firstDataRow, fallbackCol).Copy
                ws.Range(ws.Cells(firstDataRow + 1, fallbackCol), ws.Cells(wsLastRow, fallbackCol)).PasteSpecial xlPasteFormulas
                Application.CutCopyMode = False
            End If
            Dim fbRange As Range
            Set fbRange = ws.Range(ws.Cells(firstDataRow, fallbackCol), ws.Cells(wsLastRow, fallbackCol))
            fbRange.Value = fbRange.Value
        End If
    Else
        nextHelper = defHelperStart + defHelperCount
    End If

    ' --- Perform the sort ---
    Dim lastHelperCol As Long
    lastHelperCol = nextHelper - 1
    ...
    With ws.Sort
        ...
        ' Secondary: Definitions priorities (or fallback MIN date)
        If defHelperCount > 0 Then
            Dim di As Long
            For di = 0 To defHelperCount - 1
                .SortFields.Add Key:=ws.Range(ws.Cells(firstDataRow, defHelperStart + di), _
                    ws.Cells(wsLastRow, defHelperStart + di)), _
                    SortOn:=xlSortOnValues, Order:=xlAscending
            Next di
        ElseIf fallbackCol > 0 Then
            .SortFields.Add Key:=ws.Range(ws.Cells(firstDataRow, fallbackCol), ws.Cells(wsLastRow, fallbackCol)), _
                SortOn:=xlSortOnValues, Order:=xlAscending
        End If

        .SetRange ws.Range(ws.Cells(wsHdr, 1), ws.Cells(wsLastRow, lastHelperCol))
        .Header = xlYes
        .MatchCase = False
    ...
    ' Delete ALL helper columns (status + definitions/fallback)
    If lastHelperCol >= statusHelperCol Then
        ws.Range(ws.Columns(statusHelperCol), ws.Columns(lastHelperCol)).Delete
    End If
End Sub

' Map TIS source header to Our Date header for sort column lookup
Public Function GetOurDateForTISHeader(tisHeader As String) As String
    Select Case tisHeader
        Case LCase(TIS_SRC_SET): GetOurDateForTISHeader = LCase(TIS_COL_OUR_SET)
        Case LCase(TIS_SRC_SL1): GetOurDateForTISHeader = LCase(TIS_COL_OUR_SL1)
        Case LCase(TIS_SRC_SL2): GetOurDateForTISHeader = LCase(TIS_COL_OUR_SL2)
        Case LCase(TIS_SRC_SQ): GetOurDateForTISHeader = LCase(TIS_COL_OUR_SQ)
        Case LCase(TIS_SRC_CONVS): GetOurDateForTISHeader = LCase(TIS_COL_OUR_CONVS)
        Case LCase(TIS_SRC_CONVF): GetOurDateForTISHeader = LCase(TIS_COL_OUR_CONVF)
        Case LCase(TIS_SRC_MRCLS): GetOurDateForTISHeader = LCase(TIS_COL_OUR_MRCLS)
        Case LCase(TIS_SRC_MRCLF): GetOurDateForTISHeader = LCase(TIS_COL_OUR_MRCLF)
        Case Else: GetOurDateForTISHeader = ""
    End Select
End Function
